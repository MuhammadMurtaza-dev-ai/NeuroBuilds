      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-xs text-gray-500 mt-0.5 font-mono">
            Weekly GenAI analysis of hot products, price movements &amp; stale
            inventory
          </p>
        </div>
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="flex flex-wrap items-center gap-x-6 gap-y-2 text-xs font-mono text-gray-500">
            ...
            <span>
              New (24h):{" "}
              <span className="text-primary">{snapshot.newCount24h}</span>
            </span>
            <span>
              Active:{" "}
              <span className="text-primary">{snapshot.activeCount}</span>
            </span>
          </div>
          ...
          {/* Price movements */}
          <Card
            title="Price Movements (week-over-week median)"
            icon="trending_up"
          >
              ...
              <div className="space-y-2">
                {snapshot.priceMovements.map((m) => {
                  ...
                  return (
                    ...
                    >
                      <span className="w-24 sm:w-32 shrink-0 capitalize text-gray-300">
                        {m.category}
                      </span>
                      <span
                        className={`flex items-center gap-1 font-mono font-bold ${colour}`}
                      >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <Card
              title="Hot Products (demand velocity)"
                ...
                <ul className="space-y-2">
                  {snapshot.hot.map((h, i) => (
                    <li key={i} className="flex items-center gap-3 text-sm">
                      <span className="text-amber-400 font-mono text-xs w-5 shrink-0">
                        #{i + 1}
                      </span>
              ...
              {snapshot.categories.map((c) => (
                <div key={c.category} className="rounded-xl border border-white/10 bg-white/[0.03] p-3">
                  <div className="flex items-start justify-between gap-3">
                    <p className="capitalize text-sm font-semibold text-gray-200 break-words">
                      {c.category}
                    </p>
                  ...
                  <div className="mt-3 grid grid-cols-2 gap-3 text-xs">
                    <div>
                      <p className="font-mono text-gray-600 uppercase tracking-wider">Active</p>
                      <p className="mt-1 font-mono text-white">{c.activeCount}</p>
                    </div>
                    <div>
                      <p className="font-mono text-gray-600 uppercase tracking-wider">Avg Views</p>
                      <p className="mt-1 font-mono text-white">{c.avgViews ?? '-'}</p>
                    </div>
                    <div>
                      <p className="font-mono text-gray-600 uppercase tracking-wider">Min</p>
                      <p className="mt-1 font-mono text-gray-400">{rs(c.priceMin)}</p>
                    </div>
                    <div>
                      <p className="font-mono text-gray-600 uppercase tracking-wider">Median</p>
                      <p className="mt-1 font-mono text-white">{rs(c.priceMedian)}</p>
                    </div>
                    <div>
                      <p className="font-mono text-gray-600 uppercase tracking-wider">Max</p>
                      <p className="mt-1 font-mono text-gray-400">{rs(c.priceMax)}</p>
                    </div>
                  </div>
                </div>
              ))}
